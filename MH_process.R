...
# 1  ESTABLISH END OF TIMESERIES
# 2  GROUP DATA INTO PROCESSING CLUSTERS
#   2A DEFINE MATCH VARIABLES
#   2B COUNT HOW MANY CLUSTERS TO PROCESS
#   2C NOTE WHEN THERE ARE MULTIPLE RECORDS PER CLUSTER PER FR ACTIVE AT THE SAME TIME
# 3 DEFINE VARIABLES THAT DETECT REG CHANGES BASED ON STATUS TYPE
# 4 CHECK FOR CONSECUTIVELY REPEATED REGS
#   4A CREATE EMPTY LIST TO STORE DETECTED REG CHANGES
#   4B SORT BY FR_CITATION WITHIN CLUSTER (CHANGE THIS TO A FUNCTION!!!)
# 5 ADD FLAGS FOR REMOVAL, GENERAL(AKA FYI)/DETAILED, COMPLEX REGS, DEFINE SETTINGS THAT IMPLY A POTENTIALLY REDUNDANT RECORD
# 6 FILTER TO EXPLORE POTENTIALLY REDUNDANT RECORDS
# 7 ADD END DATE TO SORTED RECORDS
# 8 EXPORT DATA
...
# 5 FLATTEN LIST ADD FLAG THAT IMPLY A REDUNDANT RECORD ####

mh_detect <- mh_detect %>%
  bind_rows() %>%
  mutate(REDUNDANT = (REG_CHANGE + REG_REMOVED + MULTI_REG + GENERAL + COMPLEX + ADJUSTMENT) == 0)

# 6 FILTER TO JUST REDUNDANT RECORDS ####
...
write.csv(mh_sort, paste0("./cleaning and preprocessing/MHprocessed_", format(Sys.Date(), "%d%b%Y"), ".csv"), row.names = FALSE)
...
write.csv(mh_sort, paste0("./cleaning and preprocessing/MHprocessed_clean_", format(Sys.Date(), "%d%b%Y"), ".csv"), row.names = FALSE)
