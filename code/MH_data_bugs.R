...
mh_cleaned <- mh_cleaned %>%
  mutate(MANAGEMENT_TYPE = case_when(REGULATION_ID %in% c(439, 443, 813) ~ "QUOTA ADJUSTMENT",
                                     TRUE ~ MANAGEMENT_TYPE),
         START_MONTH = case_when(!REGULATION_ID %in% c(439, 443, 813) ~ START_MONTH),
         START_DAY = case_when(!REGULATION_ID %in% c(439, 443, 813) ~ START_DAY),
         END_DAY = case_when(!REGULATION_ID %in% c(439, 443, 813) ~ END_DAY),
         END_MONTH = case_when(!REGULATION_ID %in% c(439, 443, 813) ~ END_MONTH),
         END_YEAR = case_when(!REGULATION_ID %in% c(439, 443, 813) ~ END_YEAR))
